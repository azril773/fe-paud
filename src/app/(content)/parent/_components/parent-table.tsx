...
export default function ParentTable({ search, currentPage: initialPage }: ParentTableProps) {
    ...
    const loadData = useEffectEvent(async (page: number) => {
        ...
        const { data, totalPages: nextTotalPages, error } = await searchParents({ token, page, perPage, search })
        if (error) {
    ...
    const handleDelete = (parentId: string) => async () => {
        ...
        if (parentData.length === 1 && currentPage > 1) {
            ...
            <div className="w-full overflow-hidden rounded-sm border border-slate-200/70 bg-white/85 shadow-sm backdrop-blur-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                ...
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead className="text-[0.9rem] text-gray-600">Nama Orang Tua</TableHead>
                    ...
                    {parentData.length > 0 ? (
                        ...
                        <TableRow>
                            <TableCell colSpan={6} className="py-10 text-center text-sm text-slate-500">
                                Tidak ada data orang tua untuk ditampilkan.
                            </TableCell>
                        </TableRow>
                    )}
                </TableBody>
